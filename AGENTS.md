...
- Owner-only MCP server (`use_context` at `/mcp`), mounted by default — the owner's read/act/file surface for claude.ai/ChatGPT, the desktop apps, and the CLI clients. Locally keyless (one `claude mcp add` / `codex mcp add`, a stdio bridge for Claude Desktop); in production, clients connect over **MCP OAuth** (armed by `MCP_CONNECT_SECRET` — the deployment is its own OAuth 2.1 authorization server). Fail-closed (not a guest path). See [`docs/MCP.md`](docs/MCP.md).

## Key Files

| File | Purpose |
|------|---------|
| [`app/main.py`](app/main.py) | AgentOS entrypoint — assembles the `AgentOS` (agents, workflows, interfaces, MCP — including `mcp_auth=AgentOSBuiltinAuth(...)`, built iff `MCP_CONNECT_SECRET` is set) and the lifespan (startup checks, thread-pool sizing, create tables, register schedules, setup/close providers). Pure assembly; the steps live in their domain modules. |
| [`app/identity.py`](app/identity.py) | `OWNER_ID` parsing + `is_owner(run_context)` — the verdict the whole owner/guest model keys off. Fails closed. Drops reserved identities from `OWNER_ID` (`anon`, `__scheduler__`, and the `__oauth__:` / `sa:` namespaces — server-assigned principals are trust decisions made in code, never owner entries). |
| [`app/mcp.py`](app/mcp.py) | `context_mcp_config()` — the `MCPServerConfig` handed to `AgentOS(mcp_server=…)`. One tool (`use_context`) running the `context` agent as the owner; fail-closed owner gate (`authorize=_caller_is_owner` — token verification then owner check → 401; `__oauth__:<client_id>` principals trusted iff MCP OAuth is armed, `sa:` never) + DNS-rebinding protection (`allowed_hosts`), so it's never a guest path ([`docs/MCP.md`](docs/MCP.md)). |
...
| [`agents/inbox.py`](agents/inbox.py) | The inbound queue — `submit_update` + `my_updates` (everyone; both scoped to the caller's verified identity in code), `rundown` / `acknowledge` (owner-only; acknowledging DMs each human submitter a one-line receipt). |
| [`agents/policy.py`](agents/policy.py) | Everything decided from the verified identity: the surface (`context_instructions` / `context_tools` — owner gets the full toolset, a guest the capture surface), owner-gated skills loading, and the defense-in-depth hooks (`normalize_identity` pre, `enforce_capture_only` tool). |
| [`agents/scheduling.py`](agents/scheduling.py) | Guest scheduling — `owner_availability`, the one read a guest gets about the owner: open windows from Google's free/busy API (intervals only, never event contents). Joins the guest toolset iff `GOOGLE_*` is configured; the meeting request itself is filed via `submit_update`. |
...
| [`workflows/notify.py`](workflows/notify.py) | `dm_owner()` / `dm_user()` — the shared, ungated proactive-DM path: the reminder sweep and digests DM the owner; `acknowledge` DMs each submitter a receipt. No-op unless a bot token (+ a resolvable email) is configured. |
...
| [`evals/cases.py`](evals/cases.py) | Eval cases against `context` — the owner/guest asymmetry proven by deterministic structural gates (`boundary_is_structural`, `mcp_server_is_owner_only`, `attribution_is_unspoofable`, `digest_runs_are_read_only`) plus an adversarial guest arc and owner-competence cases (judge / reliability / capture-only checks). |
...
This is the heart of the product — read [`docs/SECURITY.md`](docs/SECURITY.md) before touching identity, tools, or the inbound queue. The one thing to internalize: the toolset is chosen **in code, from a verified identity, before the model runs**. [`context_tools()`](agents/policy.py) hands the owner the full provider surface and everyone else only the capture surface — `submit_update`, `my_updates` (their own submissions only; the filter is the verified caller identity in code), and free/busy `owner_availability` when the calendar is connected ([`agents/scheduling.py`](agents/scheduling.py)) — so a guest never even *sees* a read tool into the owner's data; the boundary is structural, not a prompt rule. `OWNER_ID` lists the identities that count as you (first is canonical); unset ⇒ capture-only for everyone (fail closed). The hooks in [`agents/policy.py`](agents/policy.py) add defense in depth.
...
| `MCP_CONNECT_SECRET` | no | — | Arms **MCP OAuth** ([`app/main.py`](app/main.py) builds `AgentOSBuiltinAuth` iff this is set, ≥16 chars): the deployment becomes its own OAuth 2.1 authorization server on `/mcp` — dynamic client registration + PKCE, consent page at `/mcp-auth/consent` gated on this secret, OAuth state hashed at rest in the existing Postgres (five auto-created tables). Clients connect by URL; the owner approves each on the consent page, and the MCP owner gate then trusts the `__oauth__:<client_id>` principals it mints ([`app/mcp.py`](app/mcp.py)). On a single-owner deploy the secret **is** an owner credential (`openssl rand -base64 32`). Unset ⇒ no OAuth (dev keyless-as-owner unchanged; prd `/mcp` reachable via control-plane JWT only). Requires `AGENTOS_URL` to be the exact public origin (HTTPS enforced for non-localhost at boot). Generated by `scripts/railway/up.sh` / `scripts/setup_context.sh`; synced by `env-sync.sh`. |
| `AGENTOS_MCP_SIGNING_KEY` | no | generated → Postgres | Pins the HS256 signing root for the MCP OAuth tokens (1h access / 30d rotating refresh) in env; ≥32 chars. Unset ⇒ generated and persisted in Postgres. **Rotating it revokes every issued token** (the kill switch); rotating `MCP_CONNECT_SECRET` only gates future consents. |
...
The MCP server is mounted at `/mcp` by [`app/main.py`](app/main.py), on by default. It exposes one tool, `use_context(message, session_id?)`, running the *real* `context` agent ([`app/mcp.py`](app/mcp.py)) as the **owner**. The point is the lowest-friction way in: [`scripts/connect.py`](scripts/connect.py) registers it with every local MCP client in one command — the **CLI clients (Claude Code, Codex)** via `claude mcp add` / `codex mcp add` (→ `http://localhost:8000/mcp`, no auth in dev), **Claude Desktop** via a small `mcp-remote` stdio bridge merged into `claude_desktop_config.json`, and **Cursor** via a native `{url}` remote entry in `~/.cursor/mcp.json` (no bridge — Cursor speaks streamable HTTP directly) — and the client then learns about @context and uses it without the owner prompting. Cloud clients (ChatGPT web, Claude web) can't reach localhost — deploy (add `https://<your-domain>/mcp` as a custom connector, OAuth) or tunnel with ngrok, the same paths as Slack.

**Production (MCP OAuth).** Against a deployed instance the front door is **MCP OAuth**: setting `MCP_CONNECT_SECRET` (≥16 chars) makes [`app/main.py`](app/main.py) build `AgentOSBuiltinAuth(url=AGENTOS_URL, secret=…, signing_key_material=AGENTOS_MCP_SIGNING_KEY)` and hand it to `AgentOS(mcp_auth=…)` — the deployment becomes its own OAuth 2.1 authorization server on `/mcp` (dynamic client registration + PKCE, consent page at `/mcp-auth/consent` gated on the secret, OAuth state hashed at rest in the existing Postgres, 1h HS256 access / 30d rotating refresh tokens). Clients connect by URL — claude.ai/ChatGPT add `https://<your-domain>/mcp` as a custom connector (OAuth client id/secret fields left empty), the CLIs use `claude mcp add … && claude mcp login context` / `codex mcp add` + `codex mcp login`, or `uvx agno connect --url https://<your-domain>` wires them all — and the owner approves each on the consent page with the secret. A consented client's token carries the reserved `__oauth__:<client_id>` principal, which the owner gate trusts iff OAuth is armed. `AGENTOS_URL` must be the exact public origin (HTTPS enforced for non-localhost at boot); rotating `AGENTOS_MCP_SIGNING_KEY` revokes every issued token (the kill switch), rotating the secret only gates future consents. [`scripts/setup_context.sh`](scripts/setup_context.sh) is the one-command front door — preflight + `railway login` → ensure `MCP_CONNECT_SECRET` + `AGENTOS_MCP_SIGNING_KEY` in `.env.production` (generated when missing) → `env-sync` → **`railway up` redeploy** (applies `numReplicas:1`; `--no-redeploy` skips it) → print the connector recipe. Full walkthrough in [`docs/MCP.md`](docs/MCP.md#production-mcp-oauth).

It's owner-only — **not** a guest path; teammates keep their Slack write path. Owner-only and fail-closed (see [`docs/SECURITY.md`](docs/SECURITY.md) L7): in prod agno verifies the bearer token first (MCP OAuth access tokens, service-account PATs, and control-plane JWTs all resolve through the same verification layer), an identity bridge stamps the verified principal as the run's `user_id`, and then the `authorize` gate (`MCPServerConfig.authorize=_caller_is_owner`) 401s anyone who is neither in `OWNER_ID` nor — iff OAuth is armed — an `__oauth__:` principal (minted only by this deployment's own consent flow; agno rejects external tokens claiming the reserved namespace; `sa:` principals always 401) — it never falls back to the guest surface. An always-on local server is a DNS-rebinding target, so host validation is on (`allowed_hosts`, anchored on localhost + the `AGENTOS_URL` host, a non-allowed Host → 400). We use AgentOS's native MCP server (`mcp_server=context_mcp_config()`), which injects the verified `user_id` into `use_context` (hidden from clients) so identity is threaded through; `use_context` still pins its runs to the canonical owner id. The per-client setup steps (Claude Code, Codex, the Claude Desktop bridge, Cursor, cloud) are in [`docs/MCP.md`](docs/MCP.md).
